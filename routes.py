...
@router.put('/users/id/')
async def put_users_id(id: int, name: Annotated[str, Query(max_length=100)], email: Annotated[str, Query(max_length=100)], password: Annotated[str, Query(max_length=100)], role: Annotated[str, Query(max_length=100)], active: str, db: Session = Depends(get_db)):
    try:
        return await service.put_users_id(db, id, name, email, password, role, active)
    except Exception as e:
        raise HTTPException(500, str(e))
